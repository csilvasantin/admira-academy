...
python3 - "$TMP/consejeros/index.html" "$TMP/advisor.css" "$TMP/academy-training-core.js" "$TMP/advisor-core.js" "$TMP/advisor.js" <<'PY'
import sys
index_path, css_path, training_path, core_path, js_path = sys.argv[1:]
html = open(index_path, encoding="utf-8").read()
css = open(css_path, encoding="utf-8").read()
